import type { Metadata, Viewport } from "next";
import { Geist, Geist_Mono } from "next/font/google";
import "./globals.css";

const geistSans = Geist({ variable: "--font-geist-sans", subsets: ["latin"] });
const geistMono = Geist_Mono({ variable: "--font-geist-mono", subsets: ["latin"] });

export const metadata: Metadata = {
  title: "Albion Guild Loot Calculator",
  description:
    "Paste an Albion Online guild chest log to value the loot at current market prices and split it evenly between participants.",
};

export const viewport: Viewport = {
  themeColor: "#10100e",
  width: "device-width",
  initialScale: 1,
};

export default function RootLayout({ children }: LayoutProps<"/">) {
  return (
    <html
      lang="en"
      className={`${geistSans.variable} ${geistMono.variable} h-full overflow-clip overscroll-none antialiased`}
    >
      <body className="flex h-full flex-col overflow-clip overscroll-none">
        {children}
      </body>
    </html>
  );
}
